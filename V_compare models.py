...
def highlight_and_sort_excel(df, filename):
    workbook = openpyxl.load_workbook(filename)
    sheet = workbook.active

    # Dynamically format columns (except the first) to show two decimal places
    for col_idx in range(2, df.shape[1] + 1):
        for row_idx in range(2, df.shape[0] + 2):  
            cell = sheet.cell(row=row_idx, column=col_idx)
            cell.number_format = '0.00'  

    # Highlight min values (excluding first column)
    for row in sheet.iter_rows(min_row=2, max_row=df.shape[0] + 1, min_col=2):
        min_val = min(cell.value for cell in row if cell.value is not None)
        for cell in row:
...
def main():
    ...
    # Corrélation
    for seuil_corr in [0.80,0.90,0.95]:
        data = feature_selection_correlation(data, colonne_cible, seuil_corr)

        for method in ['normalisation','standardisation','robust','normalisation']:

            filename = str(seuil_corr)+method

            # Normalisation / standardisation

            data = scale_data(data, method='normalisation')
            data = scale_data(data, method='standardisation')
            data = scale_data(data, method='robust')
            data = scale_data(data, method='normalisation')
            ...
            metrics_df.to_excel(f'spreadsheet/metrics_{seuil_corr}_{method}.xlsx', index=False)

            df_predictions_rounded = predictions_data.copy()
            df_predictions_rounded.iloc[:, 1:] = df_predictions_rounded.iloc[:, 1:].round(3)

            color_predictions_excel(df_predictions_rounded, f"spreadsheet/predictions_{seuil_corr}_{method}.xlsx")

            metrics_df = pd.read_excel(f'spreadsheet/metrics_{seuil_corr}_{method}.xlsx')
            for index, row in metrics_df.iterrows():
                model_name = row['Model']
                mape = row['MAPE']
                column_name = f"Corr_{seuil_corr}_{method}"
                if model_name not in final_results:
                    final_results[model_name] = {}
                final_results[model_name][column_name] = mape

    final_df = pd.DataFrame(final_results).transpose()
    final_df.index.name = 'Model'
    final_df.to_excel('spreadsheet/final_mape_comparison.xlsx', index=True)
    highlight_and_sort_excel(final_df, 'spreadsheet/final_mape_comparison.xlsx')
