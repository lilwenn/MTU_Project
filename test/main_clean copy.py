...
def split_data(df, target_column):
    ...
    - num_test_periods: number of periods to use for testing
    ...
    num_test_periods = 52
    # Ensure only numeric columns are considered for features
    numeric_df = df.select_dtypes(include=[float, int])

    # Determine the number of training periods
    train_size = len(numeric_df) - num_test_periods
...
def train_model(X_train, y_train, X_test, y_true):
    result = {}
        ...
        for scoring_name, scoring_func in  SCORING_METHODS.items():
            ...
            pipeline = Pipeline([
                ...
                ('selectkbest', SelectKBest(score_func=scoring_func if scoring_func else default_scoring_func,
                                            k= K_VALUES.get(scoring_name, default_k))),
                ('model', RandomForestRegressor())
            ])

            clf = RandomizedSearchCV(
                estimator=pipeline,
                param_distributions= HYPERPARAMETERS['RandomForestRegressor'],
                n_iter=10,
                cv=5,
                verbose=1,
                n_jobs=-1,
            ...
            y_pred = cross_val_predict(best_estimator, X_test, y_true, cv=5)
